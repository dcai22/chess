...
class Board {
 private:
    ...
    [[nodiscard]] auto getKingSquare(const PieceColour& colour) const noexcept -> std::optional<Square>;

    public:
    Board();

    auto executeMove(const LegalMove& legalMove) noexcept -> void;
    auto processMove(const LegalMove& legalMove, const int moveNum) -> void;
    auto printState() const noexcept -> void;
